...
prediction <- data.frame(Number = 3815, Nearest = 0.9, OfficeSpace = 476, Income = 35)

prediction_model <- predict(model, prediction, interval = "predict")
prediction_model
```

**I picked this model because OfficeSpace, Number, Nearest and Income are the most significant variables that effect Margin. I omitted Enrollment and Distance because they did not appear to improve accuracy of expected Margin. The model explains about half of the variation in Margin (multiple R-squared of 0.506, adjusted R-squared of 0.485) with the least amount of variables. All four predictors are statistically significant, with the largest p-value being about 0.011 for Nearest. I would not recommend this site given a predicted Margin of about 36.1%, well below the 50% threshold that defines profitable inns. Even the top of the 95% prediction interval (about 47.6%) stays below the 50% profitability bar, so the recommendation against this site is unambiguous. This site has a very high Number of competitor rooms nearby, and since each additional mile to the nearest competitor adds about 1.65 points of Margin, being only 0.9 miles from the closest competition hurts this location as well.**
